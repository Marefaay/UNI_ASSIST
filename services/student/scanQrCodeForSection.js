...
//business logic
const scanQRCodeSection = async (request, response) => {
      ...
      qrcode.callback = async (err, value) => {
        if (err) {
          ...
          if (subject) {
            ...
            if (resultData.type.toLowerCase() === "section") {
              //check if there is an attendance for lecture
              if (subject.sectionAttendance.length === 0) {
                console.log("EMPTY")
                //there is no attendance for section so add student to it
                  ...
                  { title: subject.title },
                  {
                    $push: {
                      sectionAttendance: {
                        ...
                        sectionNo:resultData.sectionNo,
...
module.exports = scanQRCodeSection;
